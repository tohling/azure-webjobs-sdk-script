...
namespace Microsoft.Azure.WebJobs.Script.Description
{
    ...
    public class ScriptFunctionInvoker : ScriptFunctionInvokerBase
    {
        ...
        internal static string ResolveBashPath()
        {
            ...
            string path = _host.SettingsManager.GetEnvironmentSetting(BashPathEnvironmentKey);
        ...
        private static string ResolveRelativePathToProgramFiles(string relativeX86Path, string relativeX64Path, string target)
        {
            ...
            if (!File.Exists(path))
            {
                programFiles = _host.SettingsManager.GetEnvironmentSetting(ProgramFiles64bitKey) ?? Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles);
