...
namespace Microsoft.Azure.WebJobs.Script.Tests
{
    public class StandbyModeTests : IDisposable
    {
        ...
        public void InStandbyMode_ReturnsExpectedValue()
        {
            ...
            using (new TestEnvironment())
            {
                _settingsManager.SetEnvironmentSetting(EnvironmentSettingNames.AzureWebsitePlaceholderMode, "1");
                Assert.Equal(true, WebScriptHostManager.InStandbyMode);

                _settingsManager.SetEnvironmentSetting(EnvironmentSettingNames.AzureWebsitePlaceholderMode, "0");
                Assert.Equal(false, WebScriptHostManager.InStandbyMode);
        ...
        {
            var home = _settingsManager.GetEnvironmentSetting(EnvironmentSettingNames.AzureWebsiteHomePath);
        ...
        private class TestEnvironment : IDisposable
        {
            ...
            {
                _settingsManager = ScriptSettingsManager.Instance;
                _prevHome = _settingsManager.GetEnvironmentSetting(EnvironmentSettingNames.AzureWebsiteHomePath);

                _home = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName());
                Directory.CreateDirectory(_home);
                _settingsManager.SetEnvironmentSetting(EnvironmentSettingNames.AzureWebsiteHomePath, _home);
            ...
            public void Dispose()
            {
                ...
                _settingsManager.SetEnvironmentSetting(EnvironmentSettingNames.AzureWebsiteHomePath, _prevHome);
                try
                {
            ...
            private void Reset()
            {
                ...
                _settingsManager.SetEnvironmentSetting(EnvironmentSettingNames.AzureWebsitePlaceholderMode, null);
            }
        }
    }
}